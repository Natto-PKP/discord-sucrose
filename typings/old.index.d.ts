 ...
 * @public
 */
declare abstract class BaseBuilder<T> {
  constructor(data?: T);
  public toJson(): T;
}
 ...
 * @public
 */
declare abstract class BaseInteractionBuilder<
  T extends Interaction | CommandInteraction | Types.ChatInputOption,
> extends BaseBuilder<T> {
   ...
   *     if (interaction.guild.id === '1234') {
   *       await interaction.reply('this guild is banned');
   *       return true;
   *     } return false;
   *   }
   * });
   * ```
   */
  public addCondition(condition: Exclude<Unpacked<T['conditions']>, null | undefined>): this;
   ...
   * ```js
   * builder.addCooldown({
   *   label: '5m for everyone',
   *   type: 'EVERYONE',
   *   value: 5 * 60000,
   * });
   * ```
   */
  public addCooldown(cooldown: Cooldown): this;
   ...
   * @example
   * ```js
   * builder.addHook('[HOOK_NAME]', ({ interaction }) => console.log(interaction.user.id));
   * ```
   */
  public addHook<H extends keyof Exclude<T['hooks'], null | undefined>>(
    hook: H,
    callback: Exclude<T['hooks'], null | undefined>[H],
  ): this;
   ...
   * builder.setBody({
   *   name: 'say',
   *   description: 'a simple description for a useless command',
   * });
   * ```
   */
  public setBody(body: T['body']): this;

  /**
   * overwrites current command permissions
   * @param conditions
   * @example
   * ```js
   ...
   * ```
   */
  public setConditions(...conditions: Exclude<Unpacked<T['cooldowns']>, null | undefined>[]): this;

  /**
   * overwrites current command cooldowns
   * @param cooldowns
   * @example
   * ```js
   * builder.setCooldowns({
   *   label: '5m for everyone',
   *   type: 'EVERYONE',
   *   value: 5 * 60000,
   * });
   * ```
   */
  public setCooldowns(...cooldowns: Cooldown[]): this;

  /**
   * add custom props
   * @param custom
   */
  public setCustom(custom: { [key: string]: any }): this;
  ...
  /**
   * Add a execution for this command
   * @param exec
   * @example
   * ```js
   * builder.setExec(({ interaction }) => {
   *   interaction.reply('ferreeeet');
   * });
   * ```
   */
  public setExec(exec: T['exec']): this;
   ...
   * @example
   * ```js
   * builder.removeCondition('banned guild');
   * ```
   */
  public removeCondition(label: string): this;

  /**
   * remove a cooldown
   * @param label
   * ```js
   * builder.removeCooldown('5m for everyone');
   * ```
   */
  public removeCooldown(label: string): this;

  /**
   * remove a hook
   * @param hook
   * ```js
   * builder.removeHook('[HOOK_NAME]');
   * ```
   */
  public removeHook(hook: keyof Exclude<T['hooks'], null | undefined>): this;
   ...
   * @example
   * ```js
   * builder.removePermission('command channels');
   * ```
   */
  public removePermission(label: string): this;
 ...
 * @public
 */
declare abstract class BaseInteractionManager<T extends InteractionData = InteractionData> {
 ...
 * @public
 */
declare class CommandInteractionManager {
  public cache: Discord.Collection<string, CommandInteractionData>;

  public constructor(sucrose: Sucrose, options: CommandInteractionManagerOptions);
   ...
   *   body: {
   *     name: 'ferret',
   *     description: 'you love ferret',
   *   },
   *
   *   exec: () => console.log('FERREEEET'),
   * });
   * ```
   * @returns
   */
  public add(command: CommandInteraction): Promise<CommandInteractionData>;
   ...
   * @example
   * ```js
   * await commands.deploy('[COMMAND_NAME]');
   * await commands.deploy('[COMMAND_NAME]', '[GUILD_ID]');
   * ```
   * @returns
   */
  public deploy(name: string, guildId?: string): Promise<Discord.ApplicationCommand<{
   ...
   * @example
   * ```js
   * await commands.undeploy('[COMMAND_NAME]');
   * await commands.undeploy('[COMMAND_NAME]', '[GUILD_ID]');
   * ```
   * @returns
   */
  public undeploy(name: string, guildId?: string): Promise<Discord.ApplicationCommand<{
    guild: Discord.GuildResolvable;
  }> | undefined | null>;
   ...
   * @example
   * ```js
   * commands.refresh('[COMMAND_NAME]');
   * ```
   * @returns
   */
  public refresh(name: string): Promise<CommandInteractionData>;
   ...
   * @example
   * ```js
   * await commands.restore('[COMMAND_NAME]');
   * await commands.restore('[COMMAND_NAME]', '[GUILD_ID]');
   * ```
   * @returns
   */
  public restore(name: string, guildId?: string) : Promise<Discord.ApplicationCommand<{
    guild: Discord.GuildResolvable;
  }> | undefined>;
}
...
export interface ChatInput extends BaseInteraction<{
  ...
  body: Discord.ChatInputApplicationCommandData | Discord.SlashCommandBuilder;
}
...
export interface ChatInputSubGroupOption extends BaseInteraction {
   ...
   * Interaction body
   */
  body: Discord.ApplicationCommandSubGroupData | Discord.SlashCommandSubcommandGroupBuilder;
}
...
export interface ChatInputSubOption extends BaseInteraction<{
   ...
   * interaction body
   */
  body: Discord.ApplicationCommandSubCommandData | Discord.SlashCommandSubcommandBuilder;
}
 ...
 * @public
 */
export interface ChatInputSubOptionData extends ChatInputSubOption {
 ...
 * @public
 */
export interface CommandInteractionManagerOptions extends GlobalOptions {
  directory: DirectoryValue<true>;
  env: EnvironmentOptions;
}
...
export interface Directories<P extends boolean = false, S extends boolean = false> {
  ...
  interactions: P extends true
    ? Partial<InteractionDirectories<S>>
    : InteractionDirectories<S>;
}
 ...
 * @public
 */
export interface InteractionDirectories<S extends boolean = false> {
  /**
...
export interface InteractionManagerOptions extends GlobalOptions {
  ...
  directories: InteractionDirectories<true>;
  env: EnvironmentOptions;
...
export interface MessageContextCommand extends BaseInteraction<{
  ...
  body: Discord.MessageApplicationCommandData | Discord.ContextMenuCommandBuilder;
}
 ...
 */
export interface MessageContextCommandData extends MessageContextCommand {
...
export interface UserContextCommand extends BaseInteraction<{
  ...
  body: Discord.UserApplicationCommandData | Discord.ContextMenuCommandBuilder;
}
 ...
 * @public
 */
export interface UserContextCommandData extends UserContextCommand {
 ...
 * @internal
 */
type Packed<T> = T extends (infer U)[] ? U[number] : T;
